...
import { saveSelectedConversationId } from '@/utils/app/storage/local/selected-conversation';
import {
...
const Home = () => {
  ...
  const handleDeleteFolder = async (folderId: string) => {
    ...
    const updatedPrompts: Template[] = prompts.map((p) => {
  ...
  useEffect(() => {
    ...
    if (!settingsLoaded) {
      const _savedSettings = getSavedSettings(user);

      console.log('savedSettings', _savedSettings);

      dispatch({
        field: 'savedSettings',
        value: _savedSettings,
      });
